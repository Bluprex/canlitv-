...
import { Maximize, Minimize, Volume2, VolumeX, Play, Pause, RefreshCw, Activity, Sun, ShieldAlert, Send, CheckCircle2, Globe, Cpu } from 'lucide-react';
...
export default function VideoPlayer({ url, playing, onTogglePlay, title, logo, onError, onFullscreenChange }: VideoPlayerProps) {
  ...
  const [errorType, setErrorType] = useState<'cors' | 'broken' | 'loading'>('loading');
  ...
  // Web'de CORS sorununu aşmak için proxy kullan
  const getStreamUrl = (originalUrl: string) => {
    if (isNative || isYouTube) return originalUrl;
    // Web'de CORS engeli olan yayınlar için proxy ekle
  ...
  useEffect(() => {
    const handleFullscreenChange = () => {
      ...
      if (isNative) {
        if (isCurrentlyFullscreen) StatusBar.hide().catch(() => {});
  ...
  }, [playing, isYouTube, url, isReady, showErrorUI]);
  ...
  useEffect(() => {
    ...
    if (Hls.isSupported() && (url.includes('m3u8') || url.includes('m3u'))) {
      hls = new Hls({ 
        ...
        lowLatencyMode: true,
        xhrSetup: (xhr) => {
          // Bazı yayıncılar için gerekli olabiliyor
      ...
      video.onerror = () => { setErrorType('broken'); setShowErrorUI(true); };
    }
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (dragStartRef.current.type === 'brightness') { setBrightness(newValue); setActiveIndicator('brightness'); }
    else { setVolume(newValue); setIsMuted(false); setActiveIndicator('volume'); }
  ...
  return (
    <div 
      ...
      className={cn(
        ...
        isFullscreen ? "fixed inset-0 h-[100dvh] w-screen z-[9999] rounded-none" : "rounded-2xl md:rounded-[2.5rem] border border-white/10 z-50",
        !showControls && "cursor-none"
      )}
      onClick={() => { if (!showControls) { setShowControls(true); startControlsTimer(); } else startControlsTimer(); }}
      onTouchStart={(e) => {
        const touch = e.touches[0];
    ...
    >
      ...
      <AnimatePresence>
        {showErrorUI && !isReady && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/95 backdrop-blur-2xl p-4 md:p-8 text-center">
            <div className="relative mb-3 md:mb-8">
               <div className={cn("absolute inset-0 blur-2xl md:blur-3xl rounded-full", errorType === 'cors' ? "bg-blue-500/20" : "bg-red-500/20")} />
               <div className={cn("relative w-14 h-14 md:w-24 md:h-24 rounded-2xl md:rounded-[2rem] border flex items-center justify-center", errorType === 'cors' ? "bg-blue-500/10 border-blue-500/20" : "bg-red-500/10 border-red-500/20")}>
                  {errorType === 'cors' ? <Globe className="w-7 h-7 md:w-12 md:h-12 text-blue-500 animate-pulse" /> : <ShieldAlert className="w-7 h-7 md:w-12 md:h-12 text-red-500 animate-pulse" />}
               </div>
            </div>
            
            <h3 className="text-xl md:text-4xl font-black uppercase tracking-tighter mb-2 md:mb-4 text-white">
              {errorType === 'cors' ? <>Erişim <span className="text-blue-500">Kısıtlı</span></> : <>Yayın <span className="text-red-500">Devre Dışı</span></>}
            </h3>
            ...
            <div className="bg-white/5 border border-white/10 px-4 py-2 md:px-6 md:py-3 rounded-full mb-4 md:mb-10">
               <p className="text-white/60 text-[10px] md:text-sm font-bold uppercase tracking-widest leading-none italic">
                 {errorType === 'cors' ? "Bu yayın web üzerinde proxy ile açılmaya çalışılıyor..." : "Bu yayın geçici olarak devre dışıdır"}
               </p>
            </div>

            <div className="flex flex-col gap-4">
              {!isNative && errorType === 'cors' && (
                <div className="flex flex-col items-center gap-2 mb-4">
                  <Cpu className="text-white/20" size={24} />
                  <p className="text-[10px] text-white/40 uppercase font-black tracking-widest">Web tarayıcısı CORS engeline takıldı.</p>
                </div>
              )}
              <button 
                ...
                className={cn(
                  ...
                  reportSuccess ? "bg-green-500 text-white shadow-green-500/20" : "bg-red-600 text-white hover:bg-red-500 shadow-red-600/20"
                )}
              >
                {isReporting ? <Activity className="animate-spin" size={16} /> : reportSuccess ? <><CheckCircle2 size={16} /> ADMİNE BİLDİRİLDİ</> : <><Send size={16} /> YAYINI BİLDİR</>}
              </button>
              
              <button onClick={(e) => { e.stopPropagation(); window.location.reload(); }} className="text-white/20 hover:text-white transition-colors text-[8px] md:text-[10px] font-black uppercase tracking-[0.3em]">TEKRAR DENE</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showControls && !showErrorUI && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-40 bg-black/30 cursor-default" onClick={(e) => { e.stopPropagation(); onTogglePlay(); startControlsTimer(); }}>
